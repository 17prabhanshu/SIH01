...
class FusionPipeline:
    ...
    async def run_susceptibility_model(self, lat: float, lon: float, elevation: float = 0.0, slope: float = 0.0) -> Tuple[Optional[float], Optional[float], str]:
        """
        Run base susceptibility model through strict ML inference contract.
        Returns: (ml_score, calibrated_prob, status_reason)
        """
        from ml.susceptibility.model import SusceptibilityModel
        import numpy as np
        import os

        # Instantiate model and attempt to load benchmark
        ...
        if os.path.exists(artifact_path):
            try:
                ...
                logger.warning(f"Failed to load ML artifact: {e}")

        # The benchmark features are [lat, lon, elevation, slope]
        ...
        if result["model_status"] == "BLOCKED" or result["probability_status"] == "UNAVAILABLE":
            logger.info(f"ML Model inference unavailable: {result['reason']}")
            # Even if OOD, we might still return the raw prediction as experimental evidence
            if result["model_status"] == "OUT_OF_DOMAIN" and "prediction" in result and result["prediction"]:
                prob = result["prediction"][0]
                calibrated = result["calibrated_probability"][0] if result["calibrated_probability"] else None
                return prob, calibrated, "EXPERIMENTAL_OOD"
            return None, None, result["model_status"]

        prob = result["prediction"][0] if result["prediction"] else None
        calibrated = result["calibrated_probability"][0] if result["calibrated_probability"] else None
        return prob, calibrated, result["model_status"]
    ...
    async def run(self, lat: float, lon: float, time_context: datetime = None) -> FusionResult:
        ...
        # 2. Run Models
        ml_score, calibrated_prob, ml_status = await self.run_susceptibility_model(lat, lon, evidence.get("elevation", 0.0), 0.0)
        ...
        trigger_prob = self.run_rainfall_trigger_model(
            ...
            ml_score  # Trigger heuristic uses ML score
        )

        # 3. Evidence Fusion
        # Fusing all evidence into a single HAZARD EVIDENCE SCORE.
        # This is strictly NOT a probability.
        ...
        # 5. Explainability
        factors = {
            "ml_susceptibility_score": ml_score,
        ...
        result = FusionResult(
            ...
            calibrated_probability=calibrated_prob, # ONLY present if model is successfully calibrated, else None
